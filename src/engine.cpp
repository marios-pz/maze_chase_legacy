#include "maze_curses/engine.h"
#include "maze_curses/diamond.h"
#include "maze_curses/malph.h"
#include "maze_curses/poter.h"

Engine::Engine(const std::string &file_path)
    : map_h(0), map_w(0), moves(0), d_will_mvin(0), game(true) {
  load_map(file_path);
...
void Engine::generate_map() {
  for (int row = 0; row < map_data.size(); row++)
    for (int col = 0; col < map_data[row].length(); col++)
      (map_data[row][col] == '*') ? add_text(row, col, "█")
                                  : add_text(row, col, ".");
}
...
void Engine::setup_credits() {
  ...
  credits_window = newwin(7, credits.length() + 8, getmaxy(stdscr) - 8,
                          getmaxx(stdscr) / 2 - (credits.length() + 8) / 3);
  ...
  if (map_file.is_open()) {
    ...
    while (std::getline(map_file, line)) {
      ...
      if (map_w != line.length()) {
        endwin();
        std::cout << "Η γραμμή " << map_h
                  << " είναι λάθος. Παρακαλώ διορθώστε το." << std::endl;
        exit(1);
      }
    }
...
void Engine::update() {
  ...
  box(window, 0, 0);
  if (winner != "") {
    add_text(getmaxy(window) / 2 - 1,
             getmaxx(window) / 2 - winner.length() / 3 + 1, winner.c_str());
  } else {
    // Τυχόν ο χρήστης κλείσει το πρόγραμμα και δεν υπάρχει νικητής.
    add_text(getmaxy(window) / 2 - 1, getmaxx(window) / 2 - 8,
             "Καλή συνέχεια!");
  }
  wrefresh(window);
  getch();
